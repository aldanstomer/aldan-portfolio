...
const project = computed(() =>
  devProjects.find((p) => p.slug === route.params.slug)
)
</script>
...
<template>
  <section v-if="project" class="w-4/5 max-w-6xl mx-auto py-20">
    <RouterLink
      :to="{ path: '/portfolio', query: { tab: 'dev' } }"
      class="inline-flex items-center text-sm text-body hover:text-heading mb-8"
    >
      ← Back to portfolio
    </RouterLink>

    <div class="mb-12">
      <p v-if="project.category" class="text-sm uppercase tracking-wider text-blue-500 mb-3">
        {{ project.category }}
      </p>

      <h1 class="text-4xl md:text-5xl font-semibold text-heading mb-4">
        {{ project.title }}
      </h1>

      <p v-if="project.summary" class="text-lg text-body max-w-3xl leading-8">
        {{ project.summary }}
      </p>
    </div>
    ...
    <div class="grid gap-10 lg:grid-cols-[1.7fr_1fr] mb-14 items-stretch">
      <div class="h-full">
        <img
            ...
            :src="project.image"
            :alt="project.title"
            class="w-full h-full rounded-2xl border border-slate-700 object-cover"
        />
    </div>
          ...
          <div v-if="project.links?.live || project.links?.github" class="pt-2">
            ...
            <div class="flex flex-wrap gap-3">
              <a
                ...
                class="inline-flex items-center rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 transition"
              >
                Live Site
              </a>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                class="inline-flex items-center rounded-xl border border-slate-600 px-4 py-2.5 text-sm font-medium text-heading hover:bg-slate-700 transition"
              >
                GitHub
              </a>
            </div>
...
<div v-if="project.features?.length" class="mb-14">
  ...
  <div class="space-y-6">
    <div
      ...
      :key="feature"
      class="flex iitems-center gap-4"
    >
      <!-- Icon + line container -->
    ...
    <div v-if="project.gallery?.length" class="mb-14">
      ...
      <div class="columns-1 md:columns-2 gap-6 space-y-6">
        <img
          ...
          class="w-full rounded-2xl border border-slate-700 object-cover break-inside-avoid"
        />
      </div>
    </div>
  </section>
