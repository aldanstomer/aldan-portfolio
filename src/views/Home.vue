...
<h1 class="text-4xl md:text-5xl font-semibold text-heading leading-tight mb-5">
  ...
  <Transition name="fade" mode="out-in">
    <span
      :key="currentTitle"
      class="text-blue-400 block xl:inline whitespace-nowrap"
    >
      {{ currentTitle }}
    </span>
  </Transition>
...
<section class="mb-16">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
    <RouterLink
      ...
      class="group bg-slate-800 border border-slate-700 rounded-2xl p-6 shadow-xs hover:-translate-y-1 hover:border-blue-600 hover:shadow-md transition"
    >
      <div class="flex items-start justify-between gap-4">
        <h3 class="text-heading font-semibold text-lg leading-snug group-hover:text-blue-300">
          {{ p.title }}
        </h3>
      ...
      <div class="flex flex-wrap gap-2">
        <span
          v-for="t in p.tags"
          :key="t"
          class="px-3 py-1 rounded-full text-sm border border-slate-600 bg-slate-900 text-body"
        >
          {{ t }}
        </span>
      </div>
